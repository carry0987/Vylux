...
同一個 `vylux` binary 目前支援三種模式：

- `all`：同時啟動 HTTP server 與 worker，適合本機與最小部署
- `server`：只處理 HTTP request、圖片同步路徑、播放代理、管理 API 與主要 metrics
- `worker`：只消費 queue 任務，並在 `WORKER_METRICS_PORT` 提供 `/metrics` 與 `/healthz`
