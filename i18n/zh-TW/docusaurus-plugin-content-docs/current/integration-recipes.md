...
這條流程適用於你的應用透過 `POST /api/video/jobs` 要求 cover 輸出，或從 `video:full` workflow 讀到 `cover` artifact。
...
1. 你的 backend 以 `X-API-Key` 送出含 `cover.enabled=true` 的影片工作，或送出包含 `cover`、`preview`、`package.hls` 的完整影片工作
...
讓播放器載入正確的 public playlist 入口，並把 content-key 存取放在短效 Bearer token 後面。

流程：

1. 你的 backend 送出 transcode job
2. 你的 backend 等待 `status=completed`
3. 你的 backend 對外公開播放入口；影片通常是 `/stream/{hash}/master.m3u8`
...
- 從 public playlist URL 載入 playlist
