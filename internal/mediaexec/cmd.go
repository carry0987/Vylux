...
// Cmd is a thin wrapper around os/exec for media commands.
type Cmd struct {
	ctx  context.Context
	bin  string
	args []string
...
// New creates a new command bound to ctx.
func New(ctx context.Context, bin string, args ...string) *Cmd {
	return &Cmd{ctx: ctx, bin: bin, args: args}
}
